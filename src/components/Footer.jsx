const Footer = () => {
  return (
    <footer id="footer-main" className="footer-main mt-auto">
      {/* ========== Footer Widgets Start ========== */}
      <section className="footer-widgets">
        <div className="container">
          <h2 className="h4 text-white mb-3 ff-dm-sans text-center text-md-start">
            Connect with us
          </h2>
          <ul className="list-unstyled social-list">
            <li>
              <a href="#" className="social-link">
                <svg
                  className="ac-icon"
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g opacity="0.6" clipPath="url(#clip0_3_35)">
                    <path
                      d="M8 1.442C10.136 1.442 10.3893 1.45 11.2333 1.48867C13.4013 1.58733 14.414 2.616 14.5127 4.768C14.5513 5.61133 14.5587 5.86467 14.5587 8.00067C14.5587 10.1373 14.5507 10.39 14.5127 11.2333C14.4133 13.3833 13.4033 14.414 11.2333 14.5127C10.3893 14.5513 10.1373 14.5593 8 14.5593C5.864 14.5593 5.61067 14.5513 4.76733 14.5127C2.594 14.4133 1.58667 13.38 1.488 11.2327C1.44933 10.3893 1.44133 10.1367 1.44133 8C1.44133 5.864 1.45 5.61133 1.488 4.76733C1.58733 2.616 2.59733 1.58667 4.76733 1.488C5.61133 1.45 5.864 1.442 8 1.442ZM8 0C5.82733 0 5.55533 0.00933333 4.702 0.048C1.79667 0.181333 0.182 1.79333 0.0486667 4.70133C0.00933333 5.55533 0 5.82733 0 8C0 10.1727 0.00933333 10.4453 0.048 11.2987C0.181333 14.204 1.79333 15.8187 4.70133 15.952C5.55533 15.9907 5.82733 16 8 16C10.1727 16 10.4453 15.9907 11.2987 15.952C14.2013 15.8187 15.82 14.2067 15.9513 11.2987C15.9907 10.4453 16 10.1727 16 8C16 5.82733 15.9907 5.55533 15.952 4.702C15.8213 1.79933 14.2073 0.182 11.2993 0.0486667C10.4453 0.00933333 10.1727 0 8 0V0ZM8 3.892C5.73133 3.892 3.892 5.73133 3.892 8C3.892 10.2687 5.73133 12.1087 8 12.1087C10.2687 12.1087 12.108 10.2693 12.108 8C12.108 5.73133 10.2687 3.892 8 3.892ZM8 10.6667C6.52733 10.6667 5.33333 9.47333 5.33333 8C5.33333 6.52733 6.52733 5.33333 8 5.33333C9.47267 5.33333 10.6667 6.52733 10.6667 8C10.6667 9.47333 9.47267 10.6667 8 10.6667ZM12.2707 2.77C11.74 2.77 11.31 3.2 11.31 3.73C11.31 4.26 11.74 4.69 12.2707 4.69C12.8007 4.69 13.23 4.26 13.23 3.73C13.23 3.2 12.8007 2.77 12.2707 2.77Z"
                      fill="white"
                      fillOpacity="0.6"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_3_35">
                      <rect width="16" height="16" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li>
              <a href="#" className="social-link">
                <svg
                  className="ac-icon"
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g opacity="0.6" clipPath="url(#clip0_3_36)">
                    <path
                      d="M16 3.03799C15.4113 3.29932 14.7787 3.47532 14.1147 3.55466C14.7927 3.14866 15.3133 2.50532 15.558 1.73866C14.924 2.11466 14.2213 2.38799 13.4733 2.53532C12.8753 1.89732 12.0213 1.49866 11.0773 1.49866C8.958 1.49866 7.40067 3.47599 7.87933 5.52866C5.152 5.39199 2.73333 4.08532 1.114 2.09932C0.254 3.57466 0.668 5.50466 2.12933 6.48199C1.592 6.46466 1.08533 6.31732 0.643333 6.07132C0.607333 7.59199 1.69733 9.01466 3.276 9.33132C2.814 9.45666 2.308 9.48599 1.79333 9.38732C2.21067 10.6913 3.42267 11.64 4.86 11.6667C3.48 12.7487 1.74133 13.232 0 13.0267C1.45267 13.958 3.17867 14.5013 5.032 14.5013C11.1267 14.5013 14.57 9.35399 14.362 4.73732C15.0033 4.27399 15.56 3.69599 16 3.03799V3.03799Z"
                      fill="white"
                      fillOpacity="0.6"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_3_36">
                      <rect width="16" height="16" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li>
              <a href="#" className="social-link">
                <svg
                  className="ac-icon"
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g opacity="0.6" clipPath="url(#clip0_3_37)">
                    <path
                      d="M13.0767 2.12269C10.674 1.95869 5.32267 1.95936 2.92333 2.12269C0.325333 2.30002 0.0193333 3.86936 0 8.00002C0.0193333 12.1234 0.322667 13.6994 2.92333 13.8774C5.32333 14.0407 10.674 14.0414 13.0767 13.8774C15.6747 13.7 15.9807 12.1307 16 8.00002C15.9807 3.87669 15.6773 2.30069 13.0767 2.12269ZM6 10.6667V5.33336L11.3333 7.99536L6 10.6667Z"
                      fill="white"
                      fillOpacity="0.6"
                    />
                  </g>
                  <defs>
                    <clipPath id="clip0_3_37">
                      <rect width="16" height="16" fill="white" />
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </li>
            <li>
              <a href="#" className="social-link">
                <svg
                  className="ac-icon"
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <g opacity="0.6">
                    <path
                      d="M6 5.33333H4V8H6V16H9.33333V8H11.7613L12 5.33333H9.33333V4.222C9.33333 3.58533 9.46133 3.33333 10.0767 3.33333H12V0H9.46133C7.064 0 6 1.05533 6 3.07667V5.33333Z"
                      fill="white"
                      fillOpacity="0.6"
                    />
                  </g>
                </svg>
              </a>
            </li>
          </ul>
        </div>
      </section>
      {/* ========== Footer Widgets Start ========== */}

      {/* ========== Footer Copyright Start ========== */}
      <div className="footer-copyright">
        <div className="container">
          <div className="row gy-2 gy-md-0 gx-4 align-items-center">
            <div className="col-md-6 text-center text-md-start">
              <p className="mb-0 copyright-text">
                © 2023 ABCandy Bangladesh Ltd.
              </p>
            </div>
            <div className="col-md-6 text-center text-md-end">
              <p className="mb-0">
                <a href="#">Terms of Use</a> & <a href="#">Privacy Policy</a>
              </p>
            </div>
          </div>
        </div>
      </div>
      {/* ========== Footer Copyright Start ========== */}
    </footer>
  );
};

export default Footer;
